...
def download_page(request):
    ...
    if not re.match(regex, url):
        return render(request, 'ytd.html' ,{'error':True})
    yt = YouTube(url)
    global streams
    streams = yt.streams
    ...
    for i in streams:
        ...
        string = str(i.resolution) + ' ' + \
            str(i.filesize_approx // 1048576) + 'mb'
    ...
    for k in range(len(ores)):
        ores[k] = ores[k] + ' ' + \
            str(streams.filter(res=ores[k]).first().filesize // 1048576) + 'mb'
...
def success(request, res):
    ...
    if request.method == 'POST' and size < 900:
        ...
        streams.filter(res=res).first().download(
            output_path=dirs, filename="video.mp4")
...
def playlist(request):
    ...
    if size < 900:
        for i in videos:
            ...
            stream = i.streams.filter(only_audio=True).first()
            stream.download(
                output_path=f'{dirs}/{title}', filename=f"{name}.mp3")
